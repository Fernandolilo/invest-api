...
public class InvestimentoControllerTest {
	...
	@MockBean
	private ClientService clientService; // Mock do serviço

	@MockBean
	private InvestimentoService service; // Mock do serviço
	...
	private final static String API = "/investimentos";
	...
	void shouldAuthenticateAndAddAuthorizationHeader() throws Exception {
		...
		BDDMockito.given(clientService.fromAuthentication(Mockito.any(AuthenticationDTO.class)))
	...
	void save() throws Exception {
		   client = Client.builder()
   	        ...
   	        investimentoNewDTO = InvestimentoNewDTO.builder()
   	        		...
   	        		.build();
   	        Investimento inv = Investimento.builder().conta(conta).valor(0.0).build();
   	        

		BDDMockito.given(mapper.map(Mockito.any(InvestimentoNewDTO.class), Mockito.eq(Investimento.class))).willReturn(inv);
		BDDMockito.given(service.save(investimentoNewDTO)).willReturn(inv);
		
		String json = new ObjectMapper().writeValueAsString(investimentoNewDTO);
